...
function Bookings() {
  ...
  const handleSearch = (event, bookingsData) => {
    const dataList = [];
    event.preventDefault();
    const email = event.target.search.value;
    setLoading(true);
    const data = bookingsData.find(booking => booking?.provider?.email === email);
      dataList.push(data);
      setSearch(dataList);
      setLoading(false);
  }
          ...
          <Container className="mt--6" fluid>
            <Card>
              <CardHeader className="border-0">
                <Row>
                  ...
                  <Col className="text-right" xs="6">
                        <form onSubmit={(event) => handleSearch(event, bookings)}>
                          <div className="d-flex justify-content-end">
                            <Input placeholder="Enter Provider Email" className="w-50" type="text" name="search" bsSize="sm" id="" />
              ...
              <Table className="align-items-center table-flush" responsive>
                ...
                <tbody>
                  {
                    search.map(booking => <tr key={booking._id}>
                      <th scope="row">
